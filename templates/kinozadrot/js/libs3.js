$(document).ready(function(){
   ...
   $('.slice-this').wTextSlicer({
	...
	$('.flikes').each(function(){
        ...
        var m = (c - b)/2,
			p = c - m;
        	if ( p > 9999 ) { p = (p / 1000).toFixed() + 'K' };
            if ( m > 9999 ) { m = (m / 1000).toFixed() + 'K' };
	...
	$('.tabs-sel').on('click', 'span:not(.current)', function() {       
		var $url = $(this).attr("data-iframe-src");       
		var $frame = $('<iframe>', {
  		 	src: $url,
   			frameborder: 0,
   			allowfullscreen: '',
   			})       
		$("#myvideobox").html($frame);       
	...
	$('#add-comm-btn1').click(function(){
		 $('#add-comm-btn1, #add-comms').slideToggle(200);
	});
        ...
        if ($urlNext !== undefined) {
			$.ajax({
                ...
                success: function(data) {                   
                    $('#bottom-nav').remove();
                    if (document.getElementById('full-comms')) {
                     	$('#full-comms').append($('#full-comms', data).html());
						$('.comm-letter').remove();
                        $('.comm-author').each(function(){
        					var a = $(this), b = a.closest('.comm-item'), c = a.text().substr(0,1),
							d = ["#c57c3b","#753bc5","#79c53b","#eb3b5a","#45aaf2","#2bcbba","#778ca3"], rand = Math.floor(Math.random() * d.length);							
...
function social_share(id) {
    var like_title = encodeURIComponent(document.title),
        ...
        like_image = encodeURIComponent($('img[itemprop="image"]').attr('src'));        
...
/* MegaPlayer */
function show_modal_megaplayer() {
	$("#div_modal_megaplayer").dialog({
		autoOpen: true,
		show: "fade",
		hide: "fade",
		width: 450,
		buttons: {
			...
			"Обновить плееры": function() {
				var new_kp_l = $("#megaplayer").val();                                          
				new_kp_l = new_kp_l.match(/\/(film|series)\/([0-9]*)\//);
				if(!new_kp_l) return;
				var new_kp = new_kp_l[2];
				$('.tabs-sel span').each(function() {    
					var url = $(this).attr("data-iframe-src");
					url = url.replace(/\/kp\/[0-9]*/, '/kp/'+new_kp);
					url = url.replace(/&kp=[0-9]*/, '&kp='+new_kp);	
					url = url.replace(/\?kp_id=[0-9]*/, '?kp_id='+new_kp);	
					$(this).attr("data-iframe-src", url);
				});

				var c_url = $('.tabs-sel span[class="current"]').attr("data-iframe-src");
				$("#myvideobox iframe").attr("src", c_url);                                              
				$("#megaplayer").val("");
				$(".add-comms, span[data-iframe-src*='youtube.com']").hide();
				$(this).dialog("close");
			}, 
		}
	});
